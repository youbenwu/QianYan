...
import com.qianyanhuyu.app_large.ui.common.swipe.RefreshLayout
...
fun <T : Any> ViewStateListPagingComponent(
    ...
    val pagingStateHolder: PagingStateHolderViewModel<T> = hiltViewModel()
...
fun getErrorMessagePair(exception: Throwable): Pair<String, Int> {
    return when (exception) {
        ...
        is UnknownHostException -> {
            Pair("网络连接失败,点击重试", R.drawable.ic_network_error)
        }
        is SocketTimeoutException -> {
            Pair("网络连接超时,点击重试", R.drawable.ic_network_error)
        }
        is JsonParseException -> {
            Pair("数据解析错误,点击重试", R.drawable.ic_network_error)
        }
        else -> {
            Pair("未知错误,点击重试", R.drawable.ic_network_error)
        }
    }
}
